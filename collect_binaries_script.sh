...
echo "🔧 Setting up directory structure for embedded Tesseract binaries..."
...
echo "  → $RESOURCES_DIR/native/windows-x64/tesseract.exe"
echo "  → $RESOURCES_DIR/native/windows-x64/libleptonica-5.dll"
echo "  → $RESOURCES_DIR/native/windows-x64/libtesseract-5.dll"
echo ""
echo "Linux binaries:"
echo "  → $RESOURCES_DIR/native/linux-x64/tesseract"
echo "  → $RESOURCES_DIR/native/linux-x64/libtesseract.so.5"
echo "  → $RESOURCES_DIR/native/linux-x64/libleptonica.so.5"
echo ""
echo "macOS binaries:"
echo "  → $RESOURCES_DIR/native/macos-x64/tesseract"
echo "  → $RESOURCES_DIR/native/macos-x64/libtesseract.5.dylib"
echo "  → $RESOURCES_DIR/native/macos-x64/libleptonica.5.dylib"
echo ""
echo "Tessdata files:"
echo "  → $RESOURCES_DIR/tessdata/eng.traineddata"
echo "  → $RESOURCES_DIR/tessdata/fra.traineddata"
echo "  → $RESOURCES_DIR/tessdata/deu.traineddata"
echo ""
echo "📋 Instructions to obtain binaries:"
echo ""
echo "Windows:"
...
echo ""
echo "⚠️  Note: Each tessdata file is ~10-15MB, so choose only languages you need!"
echo ""
echo "🚀 After placing all binaries, run: mvn clean package"